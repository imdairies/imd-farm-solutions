				...
				if (animalPopulation != null && !animalPopulation.isEmpty()) {
					...
					while (it.hasNext()) {
						...
						List<LifecycleEvent> lifeEvents = eventsLoader.retrieveSpecificLifeCycleEventsForAnimal(
								...
								startDate,
								endDate,
								Util.LifeCycleEvents.FMDVACCINE);
						if (lifeEvents != null && !lifeEvents.isEmpty()) {
							...
							message = "This animal was given vaccination " + daysSinceVaccinated + " days ago.";							
							...
							message += "This animal has not been given FMD vaccination with in the last " + ruleDto.getThirdThreshold() + " days." + ruleDto.getThirdThresholdMessage();
							animal.setThreshold3Violated(true);
						}
