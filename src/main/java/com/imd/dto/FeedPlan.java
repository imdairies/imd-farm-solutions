...
public class FeedPlan extends IMDairiesDTO {
	 ...
	 */
	private List<FeedItem> feedPlanItems;
	...
	public FeedPlan(FeedPlanBean feedPlanBean, User user) {
		...
		if (feedPlanBean.getFeedPlanItems() != null) {
			...
			this.feedPlanItems = new ArrayList<FeedItem>();
			while (it.hasNext()) {
	...
	public List<FeedItem> getFeedPlanItems() {
		return feedPlanItems;
	}
	public void setFeedPlanItems(List<FeedItem> feedPlanItems) {
	...
	private String stringify(String prefix) {
		...
		String itemsJson = "";
		int count = 0;
		if (feedPlanItems != null && !feedPlanItems.isEmpty()) {
			Iterator<FeedItem> it = feedPlanItems.iterator();
			while (it.hasNext()) {
				count++;
				FeedItem item = it.next();
				feedItemAnalysisMessages += (item.getPersonalizedFeedMessage() == null ? "" : "\n " + item.getPersonalizedFeedMessage());
				itemsJson += "{\n" + item.dtoToJson(prefix,false) + "}" + (feedPlanItems.size() == count ? "" : ",\n");
			}
		}
		return  prefix + fieldToJson("planAnalysisComments", (planAnalysisComments == null ? "": planAnalysisComments) + feedItemAnalysisMessages ) + ",\n" +
				prefix + fieldToJson("planAchievedDM", (this.planDM == null ? 0f: Float.parseFloat(Util.formatTwoDecimalPlaces(this.planDM.floatValue())))) + ",\n" +
				prefix + fieldToJson("planAchievedCP", (this.planCP == null ? 0f: Float.parseFloat(Util.formatTwoDecimalPlaces(this.planCP.floatValue())))) + ",\n" +
				prefix + fieldToJson("planAchievedME", (this.planME == null ? 0f: Float.parseFloat(Util.formatTwoDecimalPlaces(this.planME.floatValue())))) + ",\n" +
				prefix + fieldToJson("planCost", (this.planCost == null ? 0f: Float.parseFloat(Util.formatToSpecifiedDecimalPlaces(this.planCost.floatValue(),0)))) + ",\n" +
