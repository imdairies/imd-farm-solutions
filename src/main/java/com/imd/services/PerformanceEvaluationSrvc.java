...
//			returnJson = "[\n" + returnJson + "\n]";
			...
			HashMap<String, Boolean> badges = milestoneManager.getAllBadges(evalResults);
			String badgeInformation = "";
			
			if (badges != null) {
				Iterator<String> badgeNamesIt = badges.keySet().iterator();
				while (badgeNamesIt.hasNext()) {
					String badgeName = badgeNamesIt.next();
					badgeInformation += "\"" + badgeName + "\":";
					Boolean badgeObtained = badges.get(badgeName);
					if (badgeObtained != null && badgeObtained.booleanValue()) {
