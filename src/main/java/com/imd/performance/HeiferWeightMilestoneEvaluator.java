...
public class HeiferWeightMilestoneEvaluator extends MilestoneEvaluator {

	public HeiferWeightMilestoneEvaluator() {
		this.setMilestoneID(Util.PerformanceMilestone.HEIFERWEIGHT);
	}
	@Override
...
//			milestoneRule.setShortDescription(message);
			...
			milestoneRule.setShortDescription(MessageCatalogLoader.getDynamicallyPopulatedMessage(milestoneRule.getShortDescription(), values));
		...
		try {
			...
			if (animal.getCurrentAgeInDays() < heiferAgeInDaysAtMilestone) {
				...
				milestoneRule.setEvaluationResultMessage(animalTag + " is not eligible for " + this.getMilestoneID() + " performance milestone, since it is younger than " + heiferAgeInDaysAtMilestone);
				return milestoneRule;
			}
		} catch (Exception ex) {
			ex.printStackTrace();
			evaluationResultMessage = "A valid value for Heifer Age at Milestone was not found. This indicates that the AUX_INFO1 has not been properly set for the Performance Milestone: " + milestoneRule.getMilestoneID() + 
					".  This milestone can not be evaluated for this animal.";
			IMDLogger.log(evaluationResultMessage,Util.ERROR);
			couldnotBeEvaluatedMilestone.setEvaluationResultMessage(evaluationResultMessage);
			return couldnotBeEvaluatedMilestone;
		}
		Float weightOnMilestoneAge = null;
		List<LifecycleEvent> twoWtEvents = wtEventLoader.retrieveTwoRelevantWeightEvents(animal, animal.getDateOfBirth().plusDays(heiferAgeInDaysAtMilestone.intValue()));
		if (twoWtEvents == null || twoWtEvents.size() != 2) {
			if (twoWtEvents.size() == 1 && twoWtEvents.get(0).getEventTimeStamp().isEqual(animal.getDateOfBirth().plusDays(heiferAgeInDaysAtMilestone.intValue()))) {
				// case when the animal's weight was measured on the same day as the milestone threshold. We don't need to extrapolate the 
				// wegith in this case.
				weightOnMilestoneAge = Float.parseFloat(twoWtEvents.get(0).getAuxField1Value());
				...
				evaluationResultMessage = "Two valid weight records of the animal " + animalTag + " were not found. We expected to find one weight record before " + 
					Util.getDateInSQLFormat(animal.getDateOfBirth().plusDays(heiferAgeInDaysAtMilestone.intValue())) + " and one after this date. But we found " + 
			...
			weightOnMilestoneAge = deduceWeightAtMilestone(twoWtEvents.get(0), twoWtEvents.get(1),animal.getDateOfBirth(),heiferAgeInDaysAtMilestone.intValue());
		}
		
		if (weightOnMilestoneAge == null) {
			evaluationResultMessage = "Could not determine the weight of the animal " + animalTag + " at " + heiferAgeInDaysAtMilestone + " days of age. This milestone can not be evaluated for this animal.";
			IMDLogger.log(evaluationResultMessage,Util.ERROR);
			couldnotBeEvaluatedMilestone.setEvaluationResultMessage(evaluationResultMessage);
			return couldnotBeEvaluatedMilestone;
		}
		...
		milestoneRule.setEvaluationResultMessage("This animal weighed " + Util.formatTwoDecimalPlaces(weightOnMilestoneAge) + " Kgs. at " + heiferAgeInDaysAtMilestone + " days of age");
		...
		if (languageCd != null && !languageCd.equalsIgnoreCase(Util.LanguageCode.ENG)) {
			...
			dynamicValues.add(milestoneRule.getOneStarThreshold().toString());
			dynamicValues.add(milestoneRule.getAuxInfo1());
			milestoneRule.setShortDescription(MessageCatalogLoader.getDynamicallyPopulatedMessage(milestoneRule.getShortDescription(), dynamicValues));
